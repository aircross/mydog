 ...
 */
int fw_allow(const char *ip, const char *mac, int fw_connection_state)
{
	debug(LOG_DEBUG, "Allowing %s %s with fw_connection_state %d", ip, mac,
			fw_connection_state);
 ...
 * @return Return code of the command
 */
int fw_deny(const char *ip, const char *mac, int fw_connection_state)
{
	debug(LOG_DEBUG, "Denying %s %s with fw_connection_state %d", ip, mac,
			fw_connection_state);
...
void fw_sync_with_authserver(void)
{
	...
	for (p1 = p2 = client_get_first_client(); NULL != p1; p1 = p2) /** 遍历客户端信息链表，将信息通过GET请求发送给认证服务器 */
	{
		p2 = p1->next;
 ...
 * 读取终端数据，合并同步
 */
void fw_sync_clients_with_authserver(void)
{
	t_authresponse authresponse;
	char 		*token = NULL;
	char		*ip	 = NULL;
